...
def get_mnist_iter(args, kv):
    ...
    由NDArrayIter创建迭代器，一个用于训练集（训练集要打乱数据顺序shuffle=True），一个用于测试集
    """
    ...
    train = mx.io.NDArrayIter(
        to4d(train_img), train_lbl, args.batch_size, shuffle=True) #训练集
    val = mx.io.NDArrayIter(
        to4d(val_img), val_lbl, args.batch_size) #测试集
    return (train, val)
